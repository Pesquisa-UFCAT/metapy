...
<ul>
<li>\( T_{1} \) que representa uma diminuição controlada com a função: \(T{k+1} = 0.9T{k} \).</li>
<li>\( T_{2} \) que representa uma diminuição lenta com a função: \(T{k+1} = T{k}(log(100)/log(100 + k)) \).</li>
<li>\( T_{3} \) que representa uma diminuição rápida com a função: \(T{k+1} = T{k}/(1 + ((log(1 + ( \sigma \))/3( \sigma \)  \).</li>
</ul>
